import { GraphQLError, printError, formatError, } from 'graphql';
...
export function toApolloError(error, code = 'INTERNAL_SERVER_ERROR') {
    let err = error;
    if (err.extensions) {
...
export function fromGraphQLError(error, options) {
    const copy = options?.errorClass
...
export function formatApolloErrors(errors, options) {
    if (!options) {
