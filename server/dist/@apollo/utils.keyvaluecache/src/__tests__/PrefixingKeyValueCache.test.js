...
describe("PrefixingKeyValueCache", () => {
    ...
    it("PrefixesAreUnnecessaryForIsolationCache", async () => {
        const inner = new InMemoryLRUCache();
        const prefixesAreUnnecessaryForIsolationCache = PrefixingKeyValueCache.cacheDangerouslyDoesNotNeedPrefixesForIsolation(inner);
        const prefixing = new PrefixingKeyValueCache(prefixesAreUnnecessaryForIsolationCache, "prefix:");
        ...
        expect(PrefixingKeyValueCache.prefixesAreUnnecessaryForIsolation(inner)).toBe(false);
        expect(PrefixingKeyValueCache.prefixesAreUnnecessaryForIsolation(prefixesAreUnnecessaryForIsolationCache)).toBe(true);
        expect(PrefixingKeyValueCache.prefixesAreUnnecessaryForIsolation(prefixing)).toBe(true);
    });
});
