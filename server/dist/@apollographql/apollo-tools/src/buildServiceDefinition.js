import { isTypeDefinitionNode, isTypeExtensionNode, GraphQLError, buildASTSchema, Kind, extendSchema, isObjectType, } from "graphql";
...
export function buildServiceDefinition(modules) {
    const errors = [];
    ...
    for (const [typeName, typeDefinitions] of Object.entries(typeDefinitionsMap)) {
        if (typeDefinitions.length > 1) {
            errors.push(new GraphQLError(`Type "${typeName}" was defined more than once.`, typeDefinitions));
        }
    }
    for (const [directiveName, directives] of Object.entries(directivesMap)) {
        if (directives.length > 1) {
            errors.push(new GraphQLError(`Directive "${directiveName}" was defined more than once.`, directives));
        }
    }
    ...
    if (schemaDefinitions.length > 0 || schemaExtensions.length > 0) {
        ...
        for (const operationType of operationTypes) {
            ...
            if (operationTypeMap[operation]) {
                throw new GraphQLError(`Must provide only one ${operation} type in schema.`, [schemaDefinition]);
            }
            if (!(typeDefinitionsMap[typeName] || typeExtensionsMap[typeName])) {
                throw new GraphQLError(`Specified ${operation} type "${typeName}" not found in document.`, [schemaDefinition]);
            }
    ...
    for (const [typeName, typeExtensions] of Object.entries(typeExtensionsMap)) {
        if (!typeDefinitionsMap[typeName]) {
            if (Object.values(operationTypeMap).includes(typeName)) {
                ...
                errors.push(new GraphQLError(`Cannot extend type "${typeName}" because it does not exist in the existing schema.`, typeExtensions));
            }
        }
    }
    if (errors.length > 0) {
