...
describe("equality", function () {
    ...
    it("should work for arrays", function () {
        ...
        b.push(b, 2);
        assertEqual(a, b);
        assertEqual([1, /*hole*/ , 3], [1, /*hole*/ , 3]);
        assertEqual([1, /*hole*/ , 3], [1, void 0, 3]);
        // Not equal because the arrays are a different length.
        assertNotEqual([1, 2, /*hole*/ ,], [1, 2]);
    });
    ...
    it("should work for Set objects", function () {
        assertEqual(new Set().add(1).add(2).add(3).add(2), new Set().add(3).add(1).add(2).add(1));
        const obj = {};
        assertEqual(new Set().add(1).add(obj).add(3).add(2), new Set().add(3).add(obj).add(2).add(1));
        assertNotEqual(new Set(), new Set().add(void 0));
    });
    it("should work for Map objects", function () {
        assertEqual(new Map().set(1, 2).set(2, 3), new Map().set(2, 3).set(1, 2));
        assertEqual(new Map().set(1, 2).set(2, 3).set(1, 0), new Map().set(2, 3).set(1, 2).set(1, 0));
        assertNotEqual(new Map().set(1, 2).set(2, 3).set(1, 0), new Map().set(2, 3).set(1, 2).set(3, 4));
        assertEqual(new Map().set(1, new Set().add(2)), new Map().set(1, new Set().add(2)));
        assertNotEqual(new Map().set(1, new Set().add(2)), new Map().set(1, new Set().add(2).add(3)));
        const a = new Map;
        a.set(a, a);
        const b = new Map;
        b.set(a, b);
    ...
    it("should not care about repeated references", function () {
        const r = { foo: 42 };
        assertEqual([r, r, r], JSON.parse(JSON.stringify([r, r, r])));
    });
    it("should equate non-native functions with the same code", function () {
        ...
        assertNotEqual((a) => a + 1, (b) => b + 1);
        assertEqual({ before: 123, fn() { return 4; }, after: 321 }, { after: 321, before: 123, fn() { return 4; } });
        ...
        assertEqual(Function.prototype.toString.call(String.prototype.slice), Function.prototype.toString.call(Array.prototype.slice));
    });
    it("should equate async functions with the same code", function () {
        ...
        // These functions are behaviorally the same, but there's no way to
        // decide that question statically.
        assertNotEqual(async (a) => a + 1, async (b) => b + 1);
        assertEqual({ before: 123, async fn() { return 4; }, after: 321 }, { after: 321, before: 123, async fn() { return 4; } });
    });
    it("should equate generator functions with the same code", function () {
        ...
        // These functions are behaviorally the same, but there's no way to
        // decide that question statically.
        assertNotEqual(function* (a) { return yield a + 1; }, function* (b) { return yield b + 1; });
        assertEqual({ before: 123, *fn() { return 4; }, after: 321 }, { after: 321, before: 123, *fn() { return 4; } });
    });
    it("should equate async generator functions with the same code", function () {
        ...
        // These functions are behaviorally the same, but there's no way to
        // decide that question statically.
        assertNotEqual(async function* (a) { return yield a + 1; }, async function* (b) { return yield b + 1; });
        assertEqual({ before: 123, async *fn() { return 4; }, after: 321 }, { after: 321, before: 123, async *fn() { return 4; } });
    });
    ...
    describe("performance", function () {
        const limit = 1e6;
        this.timeout(20000);
        function check(a, bEqual, bNotEqual) {
