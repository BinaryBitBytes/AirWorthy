...
import { resolveGraphqlOptions, } from './graphqlOptions';
...
export function isHttpQueryError(e) {
    return e?.name === 'HttpQueryError';
}
/**
 * If options is specified, then the errors array will be formatted
 */
export function throwHttpGraphQLError(statusCode, errors, options, extensions, headers) {
    const allHeaders = {
...
export async function runHttpQuery(handlerArguments, request, csrfPreventionRequestHeaders) {
...
export async function processHTTPRequest(options, httpRequest) {
    let requestPayload;
    ...
    try {
        if (Array.isArray(requestPayload)) {
            ...
            if (response.errors && typeof response.data === 'undefined') {
                ...
                return throwHttpGraphQLError(response.http?.status || 400, response.errors, undefined, response.extensions, response.http?.headers);
            }
            if (response.http) {
                for (const [name, value] of response.http.headers) {
                    responseInit.headers[name] = value;
