...
import { processGraphQLRequest, APQ_CACHE_PREFIX, } from './requestPipeline';
...
import { ApolloServerPluginSchemaReporting, ApolloServerPluginInlineTrace, ApolloServerPluginUsageReporting, ApolloServerPluginCacheControl, ApolloServerPluginLandingPageLocalDefault, ApolloServerPluginLandingPageProductionDefault, } from './plugin';
import { pluginIsInternal } from './internalPlugin';
...
export class ApolloServerBase {
    ...
    constructor(config) {
        ...
        const { context, resolvers, schema, modules, typeDefs, parseOptions = {}, introspection, plugins, gateway, apollo, stopOnTerminationSignals, 
        // These next options aren't used in this function but they don't belong in
        // requestOptions.
        mocks, mockEntireSchema, documentStore, csrfPrevention, ...requestOptions } = this.config;
        ...
        return {
            ...
            documentStore: this.config.documentStore === undefined
                ...
                : this.config.documentStore === null
                    ? null
                    : new PrefixingKeyValueCache(this.config.documentStore, `${uuid.v4()}:`),
        };
    }
    async stop() {
        switch (this.state.phase) {
    ...
    ensurePluginInstantiation(userPlugins = [], isDev) {
        ...
        const alreadyHavePluginWithInternalId = (id) => this.plugins.some((p) => pluginIsInternal(p) && p.__internal_plugin_id__() === id);
        ...
        // Special case: inline tracing is on by default for federated schemas.
        {
            ...
            if (!alreadyHavePlugin) {
                ...
                this.plugins.push(ApolloServerPluginInlineTrace({ __onlyIfSchemaIsFederated: true }));
            }
        }
