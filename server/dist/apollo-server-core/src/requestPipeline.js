import { specifiedRules, getOperationAST, GraphQLError, validate as graphqlValidate, parse as graphqlParse, execute as graphqlExecute, Kind, } from 'graphql';
import { symbolExecutionDispatcherWillResolveField, enablePluginsForSchemaResolvers, symbolUserFieldResolver, } from './utils/schemaInstrumentation';
import { ApolloError, fromGraphQLError, SyntaxError, ValidationError, PersistedQueryNotSupportedError, PersistedQueryNotFoundError, formatApolloErrors, UserInputError, } from 'apollo-server-errors';
...
export async function processGraphQLRequest(config, requestContext) {
    ...
    if (extensions?.persistedQuery) {
        ...
        if (!config.persistedQueries || !config.persistedQueries.cache) {
            ...
            return await sendErrorResponse(new GraphQLError('Unsupported persisted query version'));
        }
        ...
        if (!(persistedQueryCache instanceof PrefixingKeyValueCache)) {
            persistedQueryCache = new PrefixingKeyValueCache(persistedQueryCache, APQ_CACHE_PREFIX);
        }
        ...
        if (query === undefined) {
            ...
            if (queryHash !== computedQueryHash) {
                return await sendErrorResponse(new GraphQLError('provided sha does not match query'));
            }
        ...
        return await sendErrorResponse(new GraphQLError('GraphQL operations must contain a non-empty `query` or a `persistedQuery` extension.'));
    }
    ...
    if (!requestContext.document) {
        ...
        if (config.documentStore) {
            ...
            Promise.resolve(config.documentStore.set(queryHash, requestContext.document)).catch((err) => logger.warn('Could not store validated document. ' + err?.message || err));
        }
    }
    ...
    async function sendErrorResponse(errorOrErrors, errorClass) {
        ...
        const response = {
            errors: formatErrors(errors.map((err) => err instanceof ApolloError && !errorClass
                ? err
                : fromGraphQLError(err, errorClass && {
                    errorClass,
                }))),
        };
        ...
        if (errors.every((err) => err instanceof PersistedQueryNotSupportedError ||
